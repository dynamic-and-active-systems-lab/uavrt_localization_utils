function [position] = localizefrombearings(X,Y,T,method)
...
elseif strcmp(method,'MEST')   
    ...
    while res > 0.001 && i <= max_iter
        ...
        bess_input = [C_bar_w-0.0001,C_bar_w+0.0001];%Create input to bessel function to take derivative; Small step back that we'll use to approx derivative.
        A = diff(log(besseli(0,bess_input)))/diff(bess_input);
        kappa = A^-1;

        t = real(sqrt(2*kappa*(1-cos(phi))));%Real is so that coder knows this has no complex parts. We know the argument of the sqrt is bounded to be greater than zero.
        psi = c*sin(t/c).*(abs(t)<c*pi); %I_(abs(t)<c*pi) is the indicator function
        wi = psi./t;
        wi(t==0) = 1; %Specified below equation eqn 4.7 in Lenth paper
